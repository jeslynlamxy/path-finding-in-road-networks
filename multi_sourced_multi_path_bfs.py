...
def expand_from_trees(graph_object, target_list, num_of_paths_required):
    ...
    while True:
        tree_dict, current_state_dict, count = expand_once(tree_dict, current_state_dict, graph_object)
        ...
        if count == 0:
            if not search_complete(current_state_dict, num_of_paths_required):
                if flagged:
                    ...
                    # if re attempt fails perform a new search as the re would need to
                    # recalibrate the leaves
                    # as new leaf before wasn't detected
...
def expand_once(tree_dict, current_state_dict, graph_object):
    count = 0
    for tree in tree_dict:
