...
Date: 12/10/2021
Topic: Main Graph Project Interface
"""
...
def get_number_of_paths(target_list):
    while True:
        try:
            ...
            print("VALID NUMBER OF PATHS YOU CAN GENERATE IS 1 TO", max_number - 1)
            user_input = int(input("USER : "))
            if 1 <= user_input <= max_number - 1:
                return user_input
            else:
                raise ValueError
        except ValueError:
...
def main():
    ...
    if utility.ensured_validity_of_search(graph_object.nodes, target_list):
        ...
        if number_of_paths == 1:
            tree_dict = multi_sourced_single_path_bfs.expand_from_multiple_nodes(graph_object, target_list)
